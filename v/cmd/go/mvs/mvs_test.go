...
func Test(t *testing.T) {
	...
	m := func(s string) Version {
		return Version{Name: s[:1], Version: s[1:]}
	}
...
func (r reqsMap) Upgrade(m Version) (Version, error) {
	...
	for k := range r {
		if k.Name == m.Name && r.Max(u.Version, k.Version) == k.Version && !strings.HasSuffix(k.Version, ".hidden") {
			u = k
		}
	}
	if u.Name == "" {
		return Version{}, fmt.Errorf("missing module: %v", Version{Name: m.Name})
	}
	return u, nil
}
...
func (r reqsMap) Previous(m Version) (Version, error) {
	var p Version
	for k := range r {
		if k.Name == m.Name && p.Version < k.Version && k.Version < m.Version && !strings.HasSuffix(k.Version, ".hidden") {
			p = k
		}
	}
	if p.Name == "" {
		return Version{Name: m.Name, Version: "none"}, nil
	}
	return p, nil
}
